...
import numpy as np
import task_generator_vid_8frame as tg
import os
import math
import argparse
import scipy as sp
...
#     results = 0.
#     eps = 1e-5
#     x = np.reshape(x, (x.shape[0], G, x.shape[1]/16,x.shape[1], x.shape[2], x.shape[3]))
#
#     x_mean = np.mean(x, axis=(2, 3, 4, 5), keepdims=True)
#     x_var = np.var(x, axis=(2, 3, 4, 5), keepdims=True)
...
def main():
    ...
    if os.path.exists(str("./model/ucf_feature_encoder_c3d_8frame_statistic" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
        feature_encoder.load_state_dict(torch.load(str("./model/ucf_feature_encoder_c3d_8frame_statistic" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
        print("load feature encoder success")
    if os.path.exists(str("./model/ucf_relation_network_c3d_8frame_statistic"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
        relation_network.load_state_dict(torch.load(str("./model/ucf_relation_network_c3d_8frame_statistic"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
    ...
    filename = "ucf_train_oneshot_c3d_8frame_statistic" + str(year) + '_' + str(month) + '_' + str(day) + ".txt"
        ...
        for episode in range(EPISODE):
            ...
            if episode%1000 == 0:
                ...
                if test_accuracy > last_accuracy:
                    ...
                    # save networks
                    torch.save(feature_encoder.state_dict(),str("./model/ucf_feature_encoder_c3d_8frame_statistic" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl"))
                    torch.save(relation_network.state_dict(),str("./model/ucf_relation_network_c3d_8frame_statistic"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl"))
